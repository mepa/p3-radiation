pro batchElecFracDensSpace, sstart, send, step
...
for number = sstart,send,step do begin
    ...
    outfile = 'plots/elec/elec_' + String(strcompress(number, /remove)) + '_z' + String(strcompress(redshift, /remove)) + '.png'    

    print, "reading density..."
    dens = loaddata_nomerge(filename,'dens')
    elecfrac =  loaddata_nomerge(filename,'elec')
    ...
    yr = [1e-5, 1e-9] ;use for LW rad

    plot, numdens, elecfrac, /xlog, /ylog, background='FFFFFF'xl, color=0, psym=3, xrange = xr, yrange=yr, xstyle=1, ystyle=1, xtitle='physical number density (cm^-3)', ytitle='free electron fraction'

    ;plot, numdens, elecfrac, /xlog, /ylog, background='FFFFFF'xl, color=0, psym=3, xrange = xr,  xstyle=1, ystyle=1
